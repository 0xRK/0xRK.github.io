...
interface Props {
  searchQuery: string
  onSearchChange: (value: string, aiSearch: boolean) => void
  onSearchSubmit: () => void
  onLogoClick: () => void
  aiSearch: boolean
...
export default function CuriaNavbar({ searchQuery, onSearchChange, onSearchSubmit, onLogoClick, aiSearch, onAiToggle }: Props) {
  ...
  useLayoutEffect(() => {
    const measure = () => {
      ...
      if (!navbarEl || !searchEl || !actionsEl || !taglineEl) { setShowTagline(false); return }
    ...
    if (window.ResizeObserver) {
      ...
      ;[navbarRef, searchRef, actionsRef, taglineRef, searchInputRef].forEach(r => r.current && ro!.observe(r.current))
    }
