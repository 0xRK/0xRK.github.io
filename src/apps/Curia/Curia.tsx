...
import { searchKeyword, searchAi } from './search'
import { getCategoryMeta } from './data/events'
import { getAllGroupCounts } from './groups'
import CuriaNavbar from './components/CuriaNavbar'
...
function eventInDateRange(ev: CuriaEvent, dateFrom: string, dateTo: string) {
...
export function Curia() {
  const [searchInput, setSearchInput]     = useState('')
  const [filters, setFilters]             = useState(DEFAULT_FILTERS)
  const [selectedEvent, setSelectedEvent] = useState<CuriaEvent | null>(null)
  const [events, setEvents]               = useState<CuriaEvent[]>([])
  const [searchResults, setSearchResults] = useState<CuriaEvent[] | null>(null)
  ...
  const doSearch = useCallback(async (q: string, useAi: boolean) => {
    if (!q) { setSearchResults(null); setSearchMeta(null); return }
    setSearching(true)
    try {
      const result = useAi
        ? await searchAi(events, q)
        : searchKeyword(events, q)
      ...
      if (result.date_range) {
        setFilters(f => ({ ...f, dateFrom: result.date_range!.start, dateTo: result.date_range!.end }))
      } else {
  ...
  const handleSearchChange = (value: string, currentAiSearch = aiSearch) => {
  ...
  const availableCategories = useMemo(() => {
    ...
    const cats: ReturnType<typeof getCategoryMeta>[] = []
  ...
  return (
    <div className="curia-app">
      ...
      <main className="page">
        {selectedEvent ? (
          <EventDetail event={selectedEvent} onBack={() => { setSelectedEvent(null); setGroupCounts(getAllGroupCounts()) }} />
        ) : (
            ...
            <section className="events-section">
              ...
              {filteredEvents.length > 0 && (
                <div className="bottom-pagination">
                  ...
                  <span className="bottom-pagination-info">{page} / {totalPages}</span>
                  <button
                    className="btn btn-outline"
