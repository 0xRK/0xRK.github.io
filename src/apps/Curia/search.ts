...
const STOP_WORDS = new Set([
  ...
  'find','show','me','events','event','something','looking','for','want',
  'unl','nebraska','lincoln','campus',
  'date','time','next','month','year','day','week','location','venue','type',
  'style','language','description','trend','availability','taste','price',
  'menu','dish','specialty','recipe','cuisine','restaurant','other','more',
  'related','keyword','keywords','synonym','synonyms','topic','topics',
  'about','like','such','also','new','any','all','get','use','used',
])

function expandStaticTerms(query: string) {
...
async function getLlmPipeline() {
  ...
  try {
    ...
    if (wasmBackend) {
      wasmBackend.wasmPaths = 'https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1/dist/'
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const opts: any = { quantized: true }
    llmPipeline = await pipeline('text2text-generation', 'Xenova/LaMini-Flan-T5-248M', opts)
    return llmPipeline
  } finally {
    llmLoading = false
  }
...
const DATE_PHRASE_RE = /\b(today|tomorrow|this\s+week(?:end)?|next\s+week|this\s+month|next\s+month|in\s+\d+\s+(?:day|week|month)s?|next\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)|(?:january|february|march|april|may|june|july|august|september|october|november|december)(?:\s+\d{1,2}(?:st|nd|rd|th)?)?)\b/gi

function stripDatePhrase(query: string) {
  return query.replace(DATE_PHRASE_RE, '').replace(/\s+/g, ' ').trim()
}

async function expandWithLlm(query: string) {
  try {
    ...
    return text
      .split(/[,;|\n]+/)
      .map((w: string) => w.trim().replace(/^\d+[.)]\s*/, '').toLowerCase())
      .flatMap((w: string) => w.split(/\s+/))
      .filter((w: string) => w.length > 2 && !/^\d/.test(w) && !STOP_WORDS.has(w))
  } catch {
    return []
  }
}
...
function parseDateRange(query: string) {
  ...
  if (mu) {
    const offset = { this: 0, current: 0, last: -1, next: 1 }[mu[1] as 'this' | 'current' | 'last' | 'next']!
    return unitRange[mu[2] as keyof typeof unitRange](offset)
  }
...
const FIELD_WEIGHTS: Record<string, number> = { name: 4, group: 3, description: 2, location: 1 }

function scoreEvent(event: CuriaEvent, terms: string[]) {
...
export function searchKeyword(events: CuriaEvent[], query: string): SearchResult {
  if (!query.trim()) return { results: events, terms: [], llmUsed: false, date_range: null, time_range: null, count: events.length }
...
export async function searchAi(events: CuriaEvent[], query: string): Promise<SearchResult> {
  if (!query.trim()) return { results: events, terms: [], llmUsed: false, date_range: null, time_range: null, count: events.length }
