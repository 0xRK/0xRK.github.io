...
const VIBE_CATEGORIES: Record<string, string[]> = {
  ...
  'Crowd': ['first-timers welcome', 'regulars only', 'strangers welcome', 'small group', 'big group', 'solo-friendly'],
  'Social': ['pregame', 'afterparty', 'dinner first', 'grab drinks', 'carpool', 'tailgate'],
  'Focus': ['study group', 'networking', 'just vibes', 'deep talks', 'creative collab', 'competitive'],
  'Logistics': ['21+', 'all ages', 'free entry', 'bringing food', 'outdoor', 'BYOB', 'sober-friendly'],
  'Misc': ['matching outfits', 'dog-friendly', 'photos', 'late night', 'morning people', 'budget-friendly', 'VIP'],
}
...
interface Props {
  ...
  userName: string
  error: string
  onConfirm: (data: { groupName: string; description: string; capacity: number; meetupDetails?: string; vibeTags?: string[] }) => void
  onClose: () => void
}

export default function GroupModal({ mode, group, eventName, userName, error, onConfirm, onClose }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onConfirm({
      ...
      capacity: isCreate ? (parseInt(capacity) || 0) : 0,
