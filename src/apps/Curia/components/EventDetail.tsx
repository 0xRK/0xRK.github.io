...
import { getGroups, createGroup, joinGroup, leaveGroup, deleteGroup, isMember, isCreator, getUser } from '../groups'
import type { CuriaGroup } from '../groups'
...
export default function EventDetail({ event, onBack }: Props) {
  ...
  const user = getUser()
  const [groups, setGroups] = useState<CuriaGroup[]>([])
  const [modal, setModal] = useState<{ mode: 'create' | 'join'; group?: CuriaGroup } | null>(null)
  ...
  const handleModalConfirm = ({ groupName, description, capacity, meetupDetails, vibeTags }: {
    groupName: string; description: string; capacity: number; meetupDetails?: string; vibeTags?: string[]
  }) => {
    try {
    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
            ...
            {event.audience?.length > 0 && (
              <div className="detail-meta-item">
                ...
                <div>
                  ...
                  <div className="detail-meta-value" style={{ fontSize: '0.85rem' }}>{event.audience.join(', ')}</div>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="groups-section">
        <div className="groups-section-header">
          ...
          <button className="btn btn-primary btn-sm" onClick={() => { setModal({ mode: 'create' }); setModalError('') }}>
            + Create Group
          </button>
        </div>

        <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 16 }}>
          Groups are saved locally in your browser. You are signed in as <strong>{user.name}</strong>.
        </p>
        ...
        {groups.length === 0 ? (
          ...
          <div className="groups-list">
            {groups.map((group) => {
              ...
              return (
                <div key={group.id} className={`group-card ${full && !member ? 'group-card-full' : ''}`}>
                  <div className="group-card-header">
                    ...
                    <div className="group-card-actions">
                      {creator ? (
                        ...
                        <button className="btn btn-ghost btn-sm" onClick={() => handleLeave(group.id)}>
                          Leave
                        </button>
                      ) : (
                        <button
                          className="btn btn-primary btn-sm"
                          onClick={() => { setModal({ mode: 'join', group }); setModalError('') }}
                          disabled={full}
                        >
                          Join
                        </button>
                  ...
                  <p className="group-card-meta">
                    Organized by {group.creatorName}
                  </p>
                  ...
                  <div className="capacity-bar-wrapper">
                    <div className="capacity-bar">
                      <div
                        ...
                        style={{ width: `${Math.min((group.members.length / group.capacity) * 100, 100)}%` }}
                      />
                    </div>
                    <span className="capacity-label">{group.members.length}/{group.capacity}</span>
                  </div>
                  ...
                  {group.vibes?.length > 0 && (
                    <div className="group-vibes">
                      {group.vibes.map(v => <span key={v} className="vibe-tag">{v}</span>)}
                    </div>
                  )}
                  ...
                  {chatOpen && (
                    <GroupMessageBoard eventId={event.id} group={group} />
                  )}
                </div>
              )
            })}
